(function(){
  ...
  function hideLegacyProjectHealth(){
    ...
    [...dashboard.querySelectorAll('.section')].forEach(sec=>{
      ...
      if(h2&&h2.textContent.replace(/\s+/g,' ').trim()==='Project Health')sec.style.display='none';
    });
  }
  async function load(){
  ...
  function boot(){
    const s=document.createElement('script');s.src=ENGINE_URL+'?v=2';s.onload=load;document.body.appendChild(s);
    const obs=new MutationObserver(()=>{window.clearTimeout(window.__heTimer);window.__heTimer=setTimeout(load,250)});
    obs.observe(document.getElementById('app')||document.body,{childList:true,subtree:true});
    window.addEventListener('sikoyek:dashboard-panel-request',load);window.setInterval(load,15000);
  }
