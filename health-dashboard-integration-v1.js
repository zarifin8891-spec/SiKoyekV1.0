(function(){
  ...
  function render(rows){
    ...
    const counts={green:0,amber:0,red:0};rows.forEach(r=>counts[r.health.level]=(counts[r.health.level]||0)+1);
    ...
    const style=document.createElement('style');style.textContent='#health-engine-v1-card .health-engine-summary{display:grid;grid-template-columns:repeat(3,1fr);gap:13px;margin-bottom:13px}#health-engine-v1-card .he-box{padding:16px 18px;border-radius:16px;border:1px solid var(--line)}#health-engine-v1-card .he-box.green{background:var(--green2);color:var(--green)}#health-engine-v1-card .he-box.amber{background:var(--amber2);color:var(--amber)}#health-engine-v1-card .he-box.red{background:var(--red2);color:var(--red)}#health-engine-v1-card .he-box .big{margin-bottom:3px}#health-engine-v1-card .he-table-card{overflow:hidden}@media(max-width:780px){#health-engine-v1-card .health-engine-summary{grid-template-columns:1fr}}';card.appendChild(style);
    const anchor=[...dashboard.querySelectorAll('.section')][0]||null;if(anchor)anchor.parentNode.insertBefore(card,anchor);else dashboard.appendChild(card);
  }
  function boot(){const s=document.createElement('script');s.src=ENGINE_URL;s.onload=load;document.body.appendChild(s);const obs=new MutationObserver(()=>{window.clearTimeout(window.__heTimer);window.__heTimer=window.setTimeout(load,250)});obs.observe(document.getElementById('app')||document.body,{childList:true,subtree:true});window.setInterval(load,15000)}
