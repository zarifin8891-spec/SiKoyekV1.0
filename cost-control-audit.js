(function(){
  if(window.__SIKOYEK_COST_CONTROL_V4__)return;
  window.__SIKOYEK_COST_CONTROL_V4__=true;
  const STYLE_ID='cost-control-v4-style',HOST_ID='cost-control-v4';
  ...
  function addStyle(){if(document.getElementById(STYLE_ID))return;const s=document.createElement('style');s.id=STYLE_ID;s.textContent=`
    ...
    #${HOST_ID} .ccv4-kpis{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:10px}
    #${HOST_ID} .ccv4-card{background:var(--card);border:1px solid var(--line);border-radius:14px;padding:9px 12px;min-width:0}
    #${HOST_ID} .ccv4-label{font-size:10px;color:var(--muted);font-weight:650;text-transform:uppercase;letter-spacing:.02em}
    #${HOST_ID} .ccv4-value{font-size:20px;line-height:1.08;font-weight:600;margin-top:5px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    #${HOST_ID} .ccv4-gap-line{display:flex;align-items:center;gap:8px;margin-top:5px;min-width:0}
    #${HOST_ID} .ccv4-gap-line .ccv4-value{margin-top:0}
    #${HOST_ID} .ccv4-gap-line .ccv4-status{margin-top:0;flex:0 0 auto}
    #${HOST_ID} .ccv4-gap-pos{color:var(--green)} #${HOST_ID} .ccv4-gap-neg{color:var(--red)}
    #${HOST_ID} .ccv4-status{display:inline-flex;padding:3px 7px;border-radius:999px;font-size:9px;font-weight:650}
    ...
    #${HOST_ID} .ccv4-panel{background:var(--card);border:1px solid var(--line);border-radius:14px;overflow:hidden}
    #${HOST_ID} .ccv4-head{padding:14px 16px 9px}
    #${HOST_ID} .ccv4-head h3{margin:0;font-size:16px} #${HOST_ID} .ccv4-head p{margin:4px 0 0;color:var(--muted);font-size:11px}
    #${HOST_ID} table{width:100%;border-collapse:collapse;table-layout:fixed}
    #${HOST_ID} th,#${HOST_ID} td{padding:9px 10px;border-top:1px solid var(--line);font-size:12px;white-space:nowrap;text-align:left}
    #${HOST_ID} th{font-size:9px;color:var(--muted);text-transform:uppercase;letter-spacing:.04em;background:#fafbfd}
    #${HOST_ID} th:nth-child(1),#${HOST_ID} td:nth-child(1){width:22%}
    #${HOST_ID} th:nth-child(2),#${HOST_ID} td:nth-child(2){width:14%}
    #${HOST_ID} th:nth-child(3),#${HOST_ID} td:nth-child(3){width:15%}
    #${HOST_ID} th:nth-child(4),#${HOST_ID} td:nth-child(4){width:15%}
    #${HOST_ID} th:nth-child(5),#${HOST_ID} td:nth-child(5){width:34%}
    ...
    @media(max-width:1050px){#${HOST_ID} .ccv4-kpis{grid-template-columns:repeat(3,minmax(0,1fr))}}
    @media(max-width:700px){#${HOST_ID} .ccv4-kpis{grid-template-columns:repeat(2,1fr)}#${HOST_ID} .ccv4-panel{overflow:auto}#${HOST_ID} table{min-width:760px}}
    @media(max-width:480px){#${HOST_ID} .ccv4-kpis{grid-template-columns:1fr}}
  ...
  function render(){
    ...
    const ratio=contractOf(d)?cashOut/contractOf(d)*100:0;
    const rapUsed=totalRap?cashOut/totalRap*100:0;
    const gap=progress-ratio;
    const contract=contractOf(d),profit=contract-totalRap;
    let status='TERKONTROL',cls='ok';if(gap<-10){status='BERISIKO';cls='risk';}else if(gap<0){status='PERLU PERHATIAN';cls='warn';}
    root.innerHTML=`<div class="ccv4-kpis">
      <div class="ccv4-card"><div class="ccv4-label">Realisasi Biaya</div><div class="ccv4-value">${money(cashOut)}</div></div>
      <div class="ccv4-card"><div class="ccv4-label">Rasio Biaya</div><div class="ccv4-value">${pct(ratio)}</div></div>
      <div class="ccv4-card"><div class="ccv4-label">RAP Terpakai</div><div class="ccv4-value">${pct(rapUsed)}</div></div>
      <div class="ccv4-card"><div class="ccv4-label">Gap Progress vs Biaya</div><div class="ccv4-gap-line"><div class="ccv4-value ${gap>=0?'ccv4-gap-pos':'ccv4-gap-neg'}">${gap>=0?'+':''}${pct(gap)}</div><span class="ccv4-status ${cls}">${status}</span></div></div>
      <div class="ccv4-card"><div class="ccv4-label">Estimasi Laba</div><div class="ccv4-value">${money(profit)}</div></div>
    </div>
    <div class="ccv4-panel"><div class="ccv4-head"><h3>Perbandingan RAP & Realisasi</h3><p>Kontrol biaya berdasarkan anggaran dan biaya aktual per kategori.</p></div>
