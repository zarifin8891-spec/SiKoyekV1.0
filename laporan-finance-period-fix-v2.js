...
(function(){
  ...
  async function refresh(){
    ...
    const kCount=document.getElementById('financeKpiCount'),kIn=document.getElementById('financeKpiIn'),kOut=document.getElementById('financeKpiOut'),kNet=document.getElementById('financeKpiNet'),body=document.getElementById('financeBody');
  ...
  function bind(){
    ...
    if(!p||p.dataset.financePeriodFixV2==='1')return;
    p.dataset.financePeriodFixV2='1';
    p.addEventListener('change',()=>setTimeout(()=>refresh().catch(()=>{}),0),true);
    ['financeFrom','financeTo','financeProject'].forEach(id=>document.getElementById(id)?.addEventListener('change',()=>refresh().catch(()=>{})));
    refresh().catch(()=>{});
  }
  function boot(){bind();let n=0;const tick=()=>{bind();if(++n<100)setTimeout(tick,100)};tick();}
