(function(){
  ...
  function repair(root,rows){
    ...
    if(values.length>=5){
      ...
      setText(values[0],String(total));setText(values[1],money(contract));setText(values[2],money(rap));setText(values[3],money(realization));setText(values[4],pct(avg));
    }
  ...
  async function load(){
    ...
    if(!client)return;
    busy=true;
    try{const {data,error}=await client.from('project_summary').select('*').order('project_code');if(error)throw error;repair(root,data||[])}catch(e){console.warn('Dashboard fallback:',e)}finally{busy=false}
  }
