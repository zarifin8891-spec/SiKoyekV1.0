/* SiKoyek Dashboard Final Override V10 — remove duplicate status from priority panel. */
(function(){
'use strict';
...
function style(){if(document.getElementById('dashboard-final-v10-style'))return;const s=document.createElement('style');s.id='dashboard-final-v10-style';s.textContent=`
...
#health-engine-v1-card .table th,#health-engine-v1-card .table td,#decision-engine-v1-card .table th,#decision-engine-v1-card .table td{box-sizing:border-box!important;min-width:0!important;vertical-align:middle!important}
...
/* Priority & action is intentionally 4 columns: Proyek, Prioritas, Masalah Utama, Tindakan. */
...
function normalizeDecision(dp){const t=dp?.querySelector('.table');if(!t)return;const hr=t.querySelector('thead tr');if(hr){const cells=[...hr.children];if(cells.length>=5){cells[2].remove()}}t.querySelectorAll('tbody tr').forEach(tr=>{const cells=[...tr.children];if(cells.length>=5)cells[2].remove();while(tr.children.length>4)tr.lastElementChild.remove()})}
