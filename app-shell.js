...
function mountModule(title,subtitle,active){document.body.innerHTML=`<div class="shell"><aside class="sidebar"><div class="brand">SiKoyek <b>V1.0</b></div><nav class="nav"><a href="index.html" class="${active==='dashboard'?'active':''}">Dashboard</a><a href="workspace.html" class="${active==='workspace'?'active':''}">Workspace</a><a href="progress.html" class="${active==='progress'?'active':''}">Progress</a><a href="rap.html" class="${active==='rap'?'active':''}">RAP</a><a href="keuangan.html" class="${active==='keuangan'?'active':''}">Keuangan</a><a href="item-pekerjaan.html" class="${active==='items'?'active':''}">Item Pekerjaan</a></nav><div class="foot">Project Control • V1.0</div></aside><main class="content"><div class="top"><div><h1>${esc(title)}</h1><p class="muted">${esc(subtitle)}</p></div><button class="btn ghost" id="logoutBtn">Keluar</button></div><div id="module"></div></main></div>`;document.getElementById('logoutBtn').addEventListener('click',logout);return document.getElementById('module')}
