...
assert.ok(integration.includes("card.id='decision-engine-v1-card'"),'Decision card id assignment missing');
assert.ok(integration.includes('Prioritas & Tindakan'),'Decision section title missing');
assert.ok(integration.includes('<th>Status</th>'),'Decision status column missing');
...
assert.equal((integration.match(/card\.id='decision-engine-v1-card'/g)||[]).length,1,'Decision card assignment should occur once');
