...
assert.ok(js.includes("m.querySelector('#f_code,#f_name')"),'Project modal lookup must target project fields, not generic #modal');
assert.ok(js.includes('snapshotProject()'),'Project snapshot helper missing');
assert.ok(js.includes('window.p6OpenCategoryMaster=openCategoryMaster'),'Category open hook not replaced');
...
assert.ok(pass6.includes("document.querySelector('#modal .modalbox .modalhead button')?.remove()"),'Pass 6 legacy close removal remains covered');
